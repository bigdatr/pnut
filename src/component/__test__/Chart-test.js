...
test('Canvas.getCanvasSize props and padding default to 0', tt => {
    const chart = shallow(<Chart data={data} xDimension="demand"><Line yDimension="supply"/></Chart>);
...
test('Canvas.getDefaultScale will default to a linearScale with no range or domain', tt => {
    const chart = shallow(<Chart data={data} xDimension="demand"><Line yDimension="supply"/></Chart>).instance();
...
test('if a scale is given a function it will be called with the scale and props', tt => {
    ...
    shallow(<Chart data={data} xDimension="demand" xScale={scale}><Line yDimension="supply"/></Chart>).instance();
...
test('it puts axis types in thier spots', tt => {
    ...
    const chart = shallow(<Chart data={data} xDimension="demand" xScale={scale}>
